...
class GdsPivotalCardRenderer
  ...
  def render_crest
    crest_size = 20.mm
    @pdf.image File.join(File.dirname(__FILE__), "..", "images", "coat-of-arms.png"), 
  ...
  def render_story_tags(story)
    ...
    if ! label_text.empty?
      @pdf.image File.join(File.dirname(__FILE__), "..", "images", "label_icon.jpg"), at: [0, @pdf.cursor], fit: [6.mm, 6.mm]
  ...
  def story_color(story)
    ...
    when "feature" then "52D017"
    when "bug" then "FF0000"
    when "chore" then "FFFF00"
    else "000000"
    end
  end
